...
def greedy_search(
    ...
    for _ in range(max_new_tokens):
        ...
        input_ids = torch.cat((input_ids, torch.tensor([most_likely_token]).unsqueeze(0)), dim=-1)
    return input_ids
...
def beam_search(
    ...
    for _ in range(max_new_tokens):
        ...
        for logp, inp_ids in candidates:
            ...
            topk_logprobs, topk_indices = torch.topk(next_token_logprobs, k=num_beans, dim=-1)
            for logprob, idx in zip(topk_logprobs.squeeze(0), topk_indices.squeeze(0)):
                next_logprob = logp + logprob.item()
                next_ids = torch.cat((inp_ids, torch.tensor([idx]).unsqueeze(0)), dim=-1)
...
def sample_top_p(
    ...
    for _ in range(max_new_tokens):
        ...
        next_token_probs = F.softmax(next_token_logits, dim=-1)
        idx = torch.multinomial(next_token_probs, num_samples=1)
        input_ids = torch.cat((input_ids, idx), -1)
    return input_ids
